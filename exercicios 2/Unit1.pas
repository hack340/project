...
begin
   if Application.MessageBox('Deseja apagar o registro selecionado',
         'Atenção',MB_YESNO + MB_ICONINFORMATION) = ID_YES then
   begin
      ZQuery2.SQL.Text :=
         'delete from cliente ' +
         'where codigo = :codigo ';
...
begin
   linha := 5;
   ImprimteTexto(RvSystem1,COLUNA1,linha,'CURSO TÉCNICO SENAC',
   ...
   linha := linha + 5;
   ImprimteTexto(RvSystem1,COLUNA1,linha,'PROGRAMAÇÃO - Delphi',
      'Arial',12,clBlack,[fsBold]);
   ...
   linha := linha + 10;
   ImprimteTexto(RvSystem1,COLUNA1,linha,'Código',
      'Arial',10,clBlue,[fsBold]);
   ImprimteTexto(RvSystem1,COLUNA2,linha,'Nome',
      'Arial',10,clBlue,[fsBold]);
   ImprimteTexto(RvSystem1,COLUNA3,linha,'Endereço',
      'Arial',10,clBlue,[fsBold]);
end;
